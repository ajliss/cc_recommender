# frozen_string_literal: true

module Utils
  class Options
    def self.card_options
      {
        'Affinity Federal Credit Union Cash Rewards' => {},
        'American Express Blue Cash Everyday' => {},
        'American Express Blue Cash Preferred' => {},
        'American Express Business Platinum' => {
          'excluded' => true,
          'no-sub' => true
        },
        'American Express Gold' => {},
        'American Express Green' => {},
        'American Express Platinum' => {},
        'Bank of America Premium Rewards' => {},
        'Bank of America Premium Rewards Elite' => {},
        'Capital One Savor One' => {},
        'Capital One Venture' => {},
        'Capital One Venture One' => {},
        'Capital One Venture X' => {},
        'Chase Freedom Unlimited' => {},
        'Chase Sapphire Preferred' => {},
        'Chase Sapphire Reserve' => {},
        'Chase Southwest Plus' => {},
        'Chase Southwest Premier' => {},
        'Chase Southwest Priority' => {},
        'Chase United Club' => {},
        'Chase United Explorer' => {},
        'Chase United Gateway' => {},
        'Chase United Quest' => {},
        'Citi American Airlines AAdvantage Executive' => {},
        'Citi American Airlines Mile Up' => {},
        'Citi American Airlines Platinum Select' => {},
        'Citi Strata' => {},
        'Citi Strata Elite' => {},
        'Citi Strata Premier' => {},
        'First National Bank of Omaha Getaway' => {},
        'Navy Federal Credit Union Flagship' => {},
        'Navy Federal Credit Union More Rewards' => {},
        'PNC Bank Cash Rewards' => {},
        'Sofi Bank Unlimited Cashback' => {},
        'U of I Community Credit Union Signature Cashback' => {},
        'UMB Simply Rewards' => {},
        'UMB Travel Select' => {},
        'US Bank Altitude Connect' => {},
        'US Bank Altitude Go' => {},
        'US Bank Cash+' => {},
        'US Bank Shopper' => {},
        'Utah Community Credit Union Everyday' => {},
        'Utah Community Credit Union Travel' => {},
        'Wells Fargo Autograph' => {},
        'Wells Fargo Autograph Journey' => {},
        'Bilt' => {}
      }
    end
  end
end
